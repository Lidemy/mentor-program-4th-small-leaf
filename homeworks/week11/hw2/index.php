<?php
  ...
  $sql = 'select p.id as id, p.title as title, p.content as content, p.created_at as created_at, c.name as name, c.id as category_id from small_leaf_blog_posts as p left join small_leaf_blog_categories as c on p.category_id = c.id where p.is_deleted is null order by p.id desc limit ? offset ?';
